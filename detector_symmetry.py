...
def sim_in_th(sim_val_score, axis, text, similarity_th):
...
def below_th(scores,similarity_th):
    below_th = False
    for score in scores:
...
def draw_star_3(img, box, rect_lattice_3, buffer, move_3_rat):
    [x1, y1], [x2, y2], [x3, y3], [x4, y4] = box
...
def draw_star_2(img, box, rect_lattice_2, buffer, move_2_rat):
...
def summary(img, signature_6, signature_3, signature_2, largestBox, rot_rect, rect_lattice_3, rect_lattice_2, buffer, move_3_rat, move_2_rat):
    plt.figure(figsize=(14, 14))
    if (signature_6 and signature_3 and signature_2):
        final_img = img.copy()
        final_img = draw_star_6(final_img, largestBox, rot_rect)
        final_img = draw_star_3(final_img, largestBox, rect_lattice_3, buffer, move_3_rat)
        final_img = draw_star_2(final_img, largestBox, rect_lattice_2, buffer, move_2_rat)
...
def run(img_name, similarity_th, gaussian_factor, morphology_kernel, adapt_th_const, buffer, move_2_rat, move_3_rat):
    ...
    r, g, b = np.random.randint(255), np.random.randint(255), np.random.randint(255)
    cv2.drawContours(img_copy, [largestBox], 0, (r, g, b), 2)
    plt.imshow(img_copy)
    plt.title('Best representative tile found', fontsize=16)
    plt.show()
    ...
    crop_img = crop_minAreaRect(img_copy_for_crop, rot_rect)
    plt.imshow(crop_img)
    plt.title("Chosen Tile", fontsize=16)
    plt.show()

    dim_0d = axis_reflective_test(crop_img, 0, rot_rect)
    axis_0_deg = sim_in_th(dim_0d, axis_0_deg, "0 degrees axis", similarity_th)
    dim_30d = axis_reflective_test(crop_img, 30, rot_rect)
    axis_30_deg = sim_in_th(dim_30d, axis_30_deg, "30 degrees axis", similarity_th)
    dim_60d = axis_reflective_test(crop_img, 60, rot_rect)
    axis_60_deg = sim_in_th(dim_60d, axis_60_deg, "60 degrees axis", similarity_th)
    dim_90d = axis_reflective_test(crop_img, 90, rot_rect)
    axis_90_deg = sim_in_th(dim_90d, axis_90_deg, "90 degrees axis", similarity_th)
    dim_120d = axis_reflective_test(crop_img, 120, rot_rect)
    axis_120_deg = sim_in_th(dim_120d, axis_120_deg, "120 degrees axis", similarity_th)
    dim_150d = axis_reflective_test(crop_img, 150, rot_rect)
    axis_150_deg = sim_in_th(dim_150d, axis_150_deg, "150 degrees axis", similarity_th)

    signature_6_axis_lst = [axis_0_deg, axis_30_deg, axis_60_deg, axis_90_deg, axis_120_deg, axis_150_deg]
    if all(signature_6_axis_lst):
        signature_6 = True

    print("*6 set to ", signature_6)
    ...
    signature_2 = sim_in_th(res1, signature_2, "lattice 1 90 degrees", similarity_th) or sim_in_th(res2, signature_2,"lattice 2 90 degrees", similarity_th) or sim_in_th(res3, signature_2, "lattice 1 0 degrees", similarity_th) or sim_in_th(res3, signature_2, "lattice 2 0 degrees", similarity_th)

    print("*2 set to ", signature_2)
    ...
    if rect_lattice_3 is None:
        ...
        res, final = summary(img, signature_6, signature_3, signature_2, largestBox, rot_rect, rect_lattice_3, rect_lattice_2, buffer, move_3_rat, move_2_rat)
        return (res, final)
